...
def read_header(fh):
    ...
    first_line = openfunc(fh).readline()
...
def get_cname_map(flag, default, ignore):
    ...
    決定要用哪些欄位名稱：
    1) 忽略 ignore
    2) 旗標 flag 覆蓋 default
    3) default 中其餘未被 ignore 或 flag 覆蓋者
    """
    clean_ignore = [clean_header(x) for x in ignore]
    ignore_set = set(clean_ignore)
    flag_keys  = set(flag.keys())

    # 先用 flags（優先）
    cname_map = {k: v for k, v in flag.items() if k not in ignore_set}

    # 再補上 default（排除 ignore 與已在 flags 的）
    for k, v in default.items():
        if k not in ignore_set and k not in flag_keys:
            cname_map[k] = v
...
def allele_merge(dat, alleles, log):
    ...
    SNP + A1 merge:
    只比對 SNP 與 A1，不檢查 A2。
    適合 h2 / S-LDSC；不適合做 rg。
    """
    # 與 HM3 資料 (alleles) merge，只保留在 HM3 裡的 SNP
    dat = pd.merge(alleles[['SNP', 'A1']], dat, how='inner', on='SNP', sort=False).reset_index(drop=True)

    # 比對 A1 是否一致（轉大寫以避免大小寫問題）
    dat['A1'] = dat['A1'].str.upper()
    dat['A1_hm'] = dat['A1_x'].str.upper() if 'A1_x' in dat else dat['A1']

    if 'A1_y' in dat:
        dat['A1_sumstats'] = dat['A1_y'].str.upper()
    else:
        dat['A1_sumstats'] = dat['A1']

    match = dat['A1_sumstats'] == dat['A1_hm']
    n_mismatch = (~match).sum()

    if n_mismatch > 0:
        log.log(f"Removed {n_mismatch} SNPs whose A1 did not match --merge-alleles (kept {match.sum()}).")

    dat = dat[match].reset_index(drop=True)

    # 清理暫時欄位，只留必要欄位
    keep_cols = [c for c in dat.columns if not c.endswith('_hm') and not c.endswith('_sumstats')]
    return dat[keep_cols]
...
def munge_sumstats(args, p=True):
    ...
    try:
        ...
        if args.daner_n:
            ...
            cname_map[frq_u] = 'FRQ'
            try:
                dan_cas = clean_header(next(x for x in file_cnames if x == 'Nca'))
            except StopIteration:
                raise ValueError('Could not find Nca column expected for daner-n format')
            try:
                dan_con = clean_header(next(x for x in file_cnames if x == 'Nco'))
            except StopIteration:
        ...
        # ---- 建立翻譯表與敘述 ----
        cname_translation = {x: cname_map[clean_header(x)] for x in file_cnames
                             if clean_header(x) in cname_map}  # note keys not cleaned
        ...
        if args.signed_sumstats is None and not args.a1_inc:
            ...
            if len(sign_cnames) > 1:
                raise ValueError('Too many signed sumstat columns. Specify which to ignore with --ignore.')
        ...
        vals = list(cname_translation.values())
        for head in vals:
            if vals.count(head) > 1:
                raise ValueError('Found {num} different {C} columns'
                                 .format(C=head, num=str(vals.count(head))))

        # ---- N 的判定 ----
        if (not args.N) and (not (args.N_cas and args.N_con)) and ('N' not in vals) and \
           (any(x not in vals for x in ['N_CAS', 'N_CON'])):
            raise ValueError('Could not determine N.')
        if (('N' in vals) or all(x in vals for x in ['N_CAS', 'N_CON'])) and ('NSTUDY' in vals):
            nstudy_keys = [x for x in list(cname_translation.keys()) if cname_translation[x] == 'NSTUDY']
            for x in nstudy_keys:
                del cname_translation[x]
        if not args.no_alleles and not all(x in vals for x in ['A1', 'A2']):
