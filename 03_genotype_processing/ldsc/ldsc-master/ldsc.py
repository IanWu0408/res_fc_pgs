...
class Logger(object):
    ...
    def __init__(self, logfile):
        self.log_fh = open(logfile, 'w', encoding='utf-8')
...
def ldscore(args, log):
    ...
    if args.annot is not None:  # read --annot
        ...
        for i,fh in enumerate(cts_fnames):
            ...
            cut_vec = ps.Series(ps.cut(vec, bins=cut_breaks, labels=labs))
            cts_levs.append(cut_vec)
            full_labs.append(labs)

        annot_matrix = ps.concat(cts_levs, axis=1)
        annot_matrix.columns = cts_colnames
        # crosstab -- for now we keep empty columns
        annot_matrix = ps.crosstab(annot_matrix.index,
    ...
    df = ps.DataFrame.from_records(np.c_[geno_array.df, lN])
    df.columns = new_colnames
    if args.print_snps:
        if args.print_snps.endswith('gz'):
            print_snps = ps.read_csv(args.print_snps, header=None, compression='gzip')
        elif args.print_snps.endswith('bz2'):
            print_snps = ps.read_csv(args.print_snps, header=None, compression='bz2')
        else:
            print_snps = ps.read_csv(args.print_snps, header=None)
    ...
    if (args.cts_bin is not None) and not args.no_print_annot:
        ...
        new_colnames = geno_array.colnames + ldscore_colnames
        annot_df = ps.DataFrame(np.c_[geno_array.df, annot_matrix])
    ...
    if annot_matrix is not None:
        # covariance matrix
        x = ps.DataFrame(annot_matrix, columns=annot_colnames)
